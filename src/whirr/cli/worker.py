...
if TYPE_CHECKING:
    ...
    from whirr.models.api import HeartbeatResponse, JobResponse
...
def _local_worker(gpu: int | None) -> None:
    ...
    config: WhirrConfig = load_config(whirr_dir)
    ...
    try:
        ...
        if orphaned:
            console.print(
                f"[yellow]Requeued {len(orphaned)} orphaned job(s)[/yellow]"
            )
            for job in orphaned:
                job_name = job.name or "unnamed"
                attempt = job.attempt + 1
                console.print(f"  - Job #{job.id}: {job_name} (attempt {attempt})")
    finally:
        conn.close()
...
def _local_worker_loop(
    ...
    """Run the main worker loop for local mode."""
    while not _shutdown_event.is_set():
        ...
        if job is None:
            ...
            _ = _shutdown_event.wait(timeout=config.poll_interval)
            continue
        ...
        def heartbeat_loop(
            heartbeat_stop: Event = heartbeat_stop,
            job_id: int = job_id,
            ...
            while not heartbeat_stop.is_set():
                try:
                    ...
                    console.print(
                        f"[yellow]Warning:[/yellow] Heartbeat failed: {exc}"
                    )
                _ = heartbeat_stop.wait(timeout=config.heartbeat_interval)
        ...
        try:
            ...
            heartbeat_stop.set()
            _ = heartbeat_thread.join(timeout=2.0)
...
def _remote_worker(server_url: str, data_dir: Path | None, gpu: int | None) -> None:
    ...
    try:
        ...
        _ = client.register_worker(worker_id, hostname, gpu_ids)
        console.print(f"[green]Worker started:[/green] {worker_id}")
        ...
        with contextlib.suppress(Exception):
            _ = client.unregister_worker(worker_id)
        client.close()
        console.print("[dim]Worker stopped[/dim]")
...
def _remote_worker_loop(
    ...
    """Run the main worker loop for remote mode."""
    from whirr.client import WhirrClientError

    while not _shutdown_event.is_set():
        try:
            job: JobResponse | None = client.claim_job(
                worker_id=worker_id,
                gpu_id=gpu,
            ...
            _ = _shutdown_event.wait(timeout=poll_interval)
            continue
        ...
        job_id = job.id
        console.print(
            f"\n[blue]Running job #{job_id}:[/blue] {job.name or job.command_argv[0]}"
        )
        ...
        def heartbeat_loop(
            ...
            while not heartbeat_stop.is_set():
                try:
                    result: HeartbeatResponse = client.renew_lease(
                        job_id=job_id,
                        worker_id=worker_id,
                        lease_seconds=lease_seconds,
                    )
                ...
                _ = heartbeat_stop.wait(timeout=heartbeat_interval)
